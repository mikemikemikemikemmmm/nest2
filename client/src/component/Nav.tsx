        ...
        <nav className="w-full flex items-end h-24 mb-4">
            ...
            <div className='pb-4 ml-24 text-lg'>
                {allNavData
                    ...
                    .map(nav => (
                        <NavLink style={{ backgroundColor: loaderData?.navRoute === nav.route ? '#F0EDE5' : 'white' }} key={nav.id} className={`inline-block w-32 text-center py-1 hover:bg-amber-300 hover:text-slate-800`} to={`/${nav.route}`}>
                            {nav.name}
                        </NavLink>))}
            </div>
        </nav>
